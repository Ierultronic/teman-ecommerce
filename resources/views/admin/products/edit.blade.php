...
<div class="bg-white shadow overflow-hidden sm:rounded-lg">
    <div class="px-4 py-5 sm:px-6">
        ...
        <p class="mt-1 max-w-2xl text-sm text-gray-500">Update the details for {{ $product->name }}</p>
    </div>
    
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="px-4 py-5 sm:p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          class="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-primary-500 focus:border-primary-500">{{ old('description', $product->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div id="variants-container" class="space-y-3">
                    @foreach($product->variants as $index => $variant)
                        <div class="flex space-x-3 p-3 border border-gray-200 rounded-md bg-gray-50">
                            <div class="flex-1">
                                <input type="text" name="variants[{{ $index }}][variant_name]" 
                                       value="{{ $variant->variant_name }}" placeholder="Variant name (e.g., Size, Color)" required
